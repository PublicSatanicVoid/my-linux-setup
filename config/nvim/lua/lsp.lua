...
local extra_py_paths_file = os.getenv("HOME") .. "/etc/site/python-extrapaths.lua"
local have_extra_py_paths, extra_py_paths = pcall(dofile, extra_py_paths_file)
if not have_extra_py_paths then
    extra_py_paths = {}
end

local excluded_py_paths_file = os.getenv("HOME") .. "/etc/site/python-excludepaths.lua"
local have_excluded_py_paths, excluded_py_paths = pcall(dofile, excluded_py_paths_file)
if not have_excluded_py_paths then
    excluded_py_paths = {}
end

local default_py_exe_file = os.getenv("HOME") .. "/etc/site/python-default-exe.lua"
local have_default_py_exe, default_py_exe = pcall(dofile, default_py_exe_file)
if not have_default_py_exe then
...
vim.api.nvim_create_autocmd('FileType', {
    ...
    callback = function(args)
        ...
        if project_root then
            ...
            client_name = "basedpyright-project-" .. vim.fn.sha256(root_dir):sub(1, 12)
